...
function writeChunkToCSV(chunk, chunkIndex, headers) {
  return new Promise((resolve, reject) => {
    ...
    const filePath = path.join(
      __dirname,
      `../../Temp/temp_chunk_A_${chunkIndex}.csv`
    );
...
const MerchantsA = async () => {
  try {
    ...
    await merchantA_FetchAndDeleteCatgory(token, merchantId);
    console.log(`merchant-A: Deleted all categories.`);

    // Step 2: Read CSV and split data into chunks of 500 rows
    const { data: csvData, headers } = await readCSV("./Temp/BranchA.csv");
