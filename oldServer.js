...
const test = async () => {
  try {
    ...
    const csvFilePathBranchA = path.join(__dirname, "../Temp/BranchA.csv");
    const csvFilePathBranchB = path.join(__dirname, "../Temp/BranchB.csv");
    const csvFilePathBranchC = path.join(__dirname, "../Temp/BranchC.csv");
    const csvFilePathBranchD = path.join(__dirname, "../Temp/BranchD.csv");
    const apiLogFile = path.join(__dirname, "../Temp/Api-Log.json");
...
cron.schedule(
  ...
  async () => {
    try {
      console.log("Running a task every day at 4 AM UK time");
      test();
      console.log(
        "\x1b[32m*=*=*=*=*=*=*=*=*=*/\x1b[0m \x1b[31mAll Products successfully Imported.\x1b[0m \x1b[32m/*=*=*=*=*=*=*=*=*=*\x1b[0m"
      );
